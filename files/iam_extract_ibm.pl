...
# V7.0    2008-10-07   # iwong  Added code to process/recognize Host_Aliases in sudoers file
...
# V7.1    2008-12-11   # iwong  Added code to get sudo version
...
#               Added code to make same output in Perl & KSH
...
#     2009-05-29      Updated code to check URT format CCC/I/ in gecos field
...
# V7.6.1 2010-05-11 # Javier Zayas       Fixed Global String code to search for the string anywhere in the GECOS, not just the beginning.
...
# V7.7.2 2011-01-13 # Vladislav Tembekov Allows user to pass an alternate country code used for labeling default customer ID's
# V7.7.3 2011-01-19 # Vladislav Tembekov Change processing command line arguments, fixed issue on HPUNIX with hostname length limitation
# V7.7.4 2011-01-25 # Vladislav Tembekov Added --ibmOnly and --customerOnly options, fixed issue with fetching groups from LDAP in autoldap mode
# V7.7.5 2011-01-28 # Vladislav Tembekov Added code to print user last logon date for Linux, Solaris
# V7.7.6 2011-02-04 # Vladislav Tembekov Added code to filter LDAP users on AIX
# V7.7.7 2011-02-15 # Vladislav Tembekov Changed trim function
# V7.7.8 2011-02-21 # Vladislav Tembekov Fixed hostname issue
# V7.7.9 2011-02-24 # Vladislav Tembekov Fixed cycling call in ProcessUserAlias and add_name functions
# V7.8   2011-03-03 # Vladislav Tembekov Added --owner flag to change output file permission
# V7.8.1 2011-03-16 # Vladislav Tembekov Fixed issue with last logon date on SanOS.Added code to filter LDAP users by host attribute in case of PAM authentication,fixed HP Unix OS name
...
print "--userCC\n";
print "Allows user to pass an alternate country code used for labeling default customer ID's\n";
print "--customerOnly\n";
...
sub init()
{
  ...
  while(@ARGV)
  {
    ...
    if($opt eq "--userCC"){$USERCC=shift @ARGV; logKnownArg($opt, $USERCC);next;}
...
sub auto_detect_centrify
{
  	...
  	if($OSNAME =~/AIX/) {
		...
		open FH_CENT, $FILE_CENT || logMsg(WARN,"Can't open $FILE_CENT : $!");
		while ($line=<FH_CENT>) {
...
sub hp_logins
{
  ...
  if($F11 ne "99999" && $F9 ne "000000")
  {
    ...
    $PWExp_Arr{$username}=POSIX::strftime("%Y%m%d", localtime($change+$F11*SEC_PER_DAY));
  }
  else
  {
    $PWExp_Arr{$username}="31 Dec 9999";
      ...
      elsif(/aix/i)
      {
          ...
          foreach (@buffer)
          {
            ...
            # current month before July and logon month before Jan
              ...
              $lastlogon = "$currentYear$monthnames{$1}$2";
              #last;
             }
...
sub report()
{
  ...
  while ( (my $username, my $usergid) = each %user_gid)
  {
    ...
    $usergecos=get_urt_format($usergecos);
  ...
  #
  {
     print OUTPUT_FILE "$URTCUST|S|$HOSTNAME|$OSNAME|$username||$usergecos|$userstate|$userllogon|$groupField|$privField|$PWChg\n";
    logDebug("Report: $URTCUST|S|$HOSTNAME|$OSNAME|$username||$usergecos|$userstate|$userllogon|$groupField|$privField|$PWChg");
...
sub printsig()
{
  ...
  if ($SIG_TSCM) {
    ...
    $NOTREALID = "NOTaRealID-AGCustom-FUS";
  }
  else {
  ...
  {
    print OUTPUT_FILE "S|$URTCUST|S|$HOSTNAME|$OSNAME|$NOTREALID|$UATstr|000/V///$myAUDITDATE:FN=$0:VER=$VERSION:CKSUM=$CKSUM:SUDO=$SUDOVER||$TIMEZONE|$knowpar|$EXIT_CODE|||||||\n";
  }
  else
  #MEF3 \93customer|identifier type|server identifier/application identifier|OS name/Application name|account|UICMode|userID convention data|state|l_logon |group|privilege\94
  {
    print OUTPUT_FILE "$URTCUST|S|$HOSTNAME|$OSNAME|$NOTREALID|$UATstr|000/V///$myAUDITDATE:FN=$0:VER=$VERSION:CKSUM=$CKSUM:SUDO=$SUDOVER||$TIMEZONE|$knowpar|$EXIT_CODE\n";
...
sub get_ldgrp
{
 ...
 if($TLS == 1) {
        ...
        if($LDAPFILE eq "")
        {
                `$LDAPCMD -LLL -h ldaps://$LDAPSVR -b $LDAPBASE -p $LDAPPORT objectClass=posixGroup  cn gidNumber memberUid -Z > $tmpfile `;

                logDebug("Command used to get LDAP Group1:$LDAPCMD -LLL -h ldaps://$LDAPSVR -b $LDAPBASE -p $LDAPPORT objectClass=posixGroup  cn gidNumber memberUid -Z");
        }
        else
         {
                `$LDAPCMD -LLL -h ldaps://$LDAPSVR -b $LDAPBASEGROUP -p $LDAPPORT $LDAPADDITIONAL objectClass=$LDAPGROUPOBJCLASS cn gidNumber memberUid -Z > $tmpfile `;

        logDebug("Command used to get LDAP Group2:$LDAPCMD -LLL -h ldaps://$LDAPSVR -b $LDAPBASEGROUP -p $LDAPPORT $LDAPADDITIONAL objectClass=$LDAPGROUPOBJCLASS cn gidNumber memberUid -Z");
        }
        ...
        else
        {
                ...
                logDebug("Command used to get LDAP Group4:$LDAPCMD -LLL -h $LDAPSVR -b $LDAPBASEGROUP -p $LDAPPORT $LDAPADDITIONAL objectClass=$LDAPGROUPOBJCLASS cn gidNumber memberUid");
        }
}
