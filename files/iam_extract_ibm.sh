...
function logHeader
{
  ...
  logDiv
  logInfo "IAM Global IBM OS Extractor"
  logDiv
}
	...
	user_with_domain=""
      ...
      if [[ $SCMFORMAT -eq 1 ]]; then
        echo "$HOSTNAME\t$OS\t$myAUDITDATE\t$userid\t$gecos\t$scmstate\t$userllogon\t$groupField\t$privField" >> $OUTPUTFILE
          ...
          logDebug "MEF3:$CUSTOMER|S|$HOSTNAME|$OS|$userid||$gecos|$userstate|$userllogon|$groupField|$privField|$PWChg"
      fi
}  
...
if [[ $LDAP -eq 1 ]]; then
    ...
    if [[ $TLS -eq 1 ]]; then
        ...
        attr=`ldapsearch -x -H ldaps://$LDAPSVR:$LDAPPORT -b $LDAPBASE`
...
if [[ $TLS -eq 1 ]]; then
      logDebug "process_TLS_LDAP_users: "
      DATA=`ldapsearch -x -H ldaps://$LDAPSVR:$LDAPPORT -b $LDAPBASE -Z >> /tmp/ldap_users`
...
do
  ...
  -h|-help ) 
    ...
    echo "USAGE: iam_extract_ibm.ksh [-f sudoers_file] [-r results_file] [-p passwd_file]"
    ...
    echo "                           [-C <userCC>] [-K] [-I] [-O <owner>] [-D] [-G] [-d]" 
    ...
    echo "  -C   Allows user to pass an alternate country code used for labeling default customer ID's"
    echo "  -G   Disable GSA"
    echo "  -d   Debug mode"    
...
TCM ) 
  ...
  ;;
FUS )
  NOTAREALID="NOTaRealID-AGCustom-FUS"
  ;;
*)
